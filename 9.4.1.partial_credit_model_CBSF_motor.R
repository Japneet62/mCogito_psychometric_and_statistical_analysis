##################################################################################################### 
############## Rasch model - CBSF_motor Fluid intellegence/geometric pattern recognition  ################# 
##################################################################################################### 
...
CBSF_motor <- CBSF[,c(1:14)]
  
CBSF_motor_age <- cbind(CBSF_motor, age = CBSF_grp$age) 
CBSF_motor_sex <- cbind(CBSF_motor, gender = CBSF_grp$sex_M1_F0)

## start saving all the plots created in the script below in a pdf  ----
pdf(file = "/Users/japneetbhatia/desktop/thesis/14.STATISTICAL_RESULTS/plots/CBSF_motor_pcm.pdf",   # The directory you want to save the file in
...
age_cat <- factor(CBSF_motor_age$age <= median(CBSF_motor_age$age), 
                  labels = c("50-60", "<60")) 

# - trial code to divide data into separate age groups  ----
age_cat2 <- CBSF_motor_age %>% 
  mutate(
    # Create categories
...
sex_cat <- factor(CBSF_motor_sex$gender, levels = c("M", "F"), 
                  labels = c("Male", "Female"))  

# # another method to check if the division is correct 
# CBSF_motor_sex %>% group_by(gender) %>% summarise(n = n()) 
